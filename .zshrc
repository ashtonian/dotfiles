...
# Claude CLI helpers (session management, documentation workflows, code quality)
if [[ -f $HOME/.dotfiles/lib_sh/claude.sh ]]; then
  source $HOME/.dotfiles/lib_sh/claude.sh
fi

# Atuin shell history (cross-device sync)
# Install: brew install atuin && atuin login -u <username>
if command -v atuin &>/dev/null; then
  eval "$(atuin init zsh)"
  # Optional: disable up-arrow binding if you prefer ctrl-r only
...
plugins=(
    ...
    extract
    gem
    # git - removed in favor of gitfast (duplicate functionality, gitfast is faster)
    git-auto-fetch
    git-extras
    git-prompt
    gitfast
...
##############################################################################
# Post Completion installs (idempotent - only runs once)
##############################################################################

# colorls installation marker
COLORLS_MARKER="$HOME/.local/share/dotfiles/.colorls_installed"
if [[ ! -f "$COLORLS_MARKER" ]]; then
  if ! gem list colorls -i >/dev/null 2>&1; then
    echo "colorls gem not found, installing..."
    gem install colorls && {
      mkdir -p "$(dirname "$COLORLS_MARKER")"
      touch "$COLORLS_MARKER"
    }
  else
    mkdir -p "$(dirname "$COLORLS_MARKER")"
    touch "$COLORLS_MARKER"
...
##############################################################################
# Fun Prompt (optional - set SHOW_COWSAY=true to enable)
##############################################################################

if [[ "${SHOW_COWSAY:-false}" == "true" ]] && command -v cowsay &>/dev/null && command -v fortune &>/dev/null && command -v lolcat &>/dev/null; then
  animals=(
